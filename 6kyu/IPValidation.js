...
function isValidIP(str) {
    if (str.includes('\n') || str.includes('e') || str.includes('..')) return false
    let test = str.split('.')
    if (test.length!=4) return false
    console.log(test)
    for (let num of test){
        //is int
        if (Number.isInteger(+num)==false || num==='') return false
        //num between 0 and 255
        if (num<0 || num>255) return false
        //no leading 0s
        if (num[0]==0 && num.length!=1) return false
        //no leading or trailing spaces
        if (num.trim()!==(num)) return false
    }
    return true
}
console.log(isValidIP("12.255.56."    ),  false);
