//given a list of names as a single string, return a string: uppercased, last name first, alpha by last name 

function meeting(s){
    let ans=s.toUpperCase().split(';')
    return ans.map(w=>w.split(':').reverse().join(', ')).map(w=>'('+w+')').sort().join('')
}
